...
const TelemetryMultiChart2: React.FC<Props> = ({ token, entityId, startTs, endTs, refreshMs = 60000 }) => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const resp = await thingsboard.telemetry().getTimeseries(
          ...
          { keys: "air_temp,air_hum,lux,soil_moist", startTs, endTs, interval: 60000, limit: 10000, useStrictDataTypes: false }
        );
        if (!mounted) return;
    ...
    run();
    if (refreshMs && refreshMs > 0) timer = setInterval(run, refreshMs);
  ...
  useEffect(() => {
    const canvas = document.getElementById("multiChartCanvas2") as HTMLCanvasElement | null;
    if (!canvas) return;
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="relative">
          <canvas id="multiChartCanvas2" width={width} height={height} onMouseMove={handleMouseMove} />
          {hover?.values && (
...
export default TelemetryMultiChart2;