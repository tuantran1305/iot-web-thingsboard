...
import InputThreshold from "@/app/(root)/(routes)/(dashboard)/components/input-threshold";
import LatestTelemetryCard from "@/app/(root)/(routes)/(dashboard)/components/latest-telemetry-card";
import TelemetryTable from "@/app/(root)/(routes)/(dashboard)/components/telemetry-table";
import TelemetryChart from "@/app/(root)/(routes)/(dashboard)/components/telemetry-chart";
import TelemetryMultiChart from "@/app/(root)/(routes)/(dashboard)/components/telemetry-multi-chart";
...
const DashboardPage = () => {
  ...
  // Maps component removed per request

  const onClickDevice = useCallback(async (data: any) => {
    const token = localStorage.getItem("token");
      {/* Map and SafeZoneEditor removed */}
